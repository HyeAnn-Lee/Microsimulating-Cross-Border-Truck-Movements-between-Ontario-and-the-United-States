...
def Main():
    ...
    Veh_attributes = Vissim.Net.Vehicles.GetMultipleAttributes(('RoutDecType', 'RoutDecNo', 'VehType', 'No', 'CurParkLot'))
    if len(Veh_attributes) > 0: # Check if there are any vehicles in the network:
        ...
        # For all C2X vehicles: check if there is an incident | incident is modelled as parking routing decision #1
        for cnt_C2X_veh in range(len(Veh_C2X_attributes)):
            if Veh_C2X_attributes[cnt_C2X_veh][0] == 'PARKING' and Veh_C2X_attributes[cnt_C2X_veh][1] == 1: # vehicle has an incident (parking routing decision #1)
                ...
                for cnt_Veh_Rec_Message in range(len(Veh_attributes_Rec_Message)):
                    ...
                    if pos_cur is not None and (veh_type_cur == Vehicle_Type_C2X_no_message or veh_type_cur == Vehicle_Type_C2X_HasCurrentMessage) and pos_cur < Pos_Veh_SM and Pos_Veh_SM > pos_C2X_cur: # check if vehicle has C2X & position of C2X message is downstream & there is no other further downstream message active
                        if des_speed_cur == speed_incident:
                            # if the attribute 'DesSpeed' was already set to 'speed_incident', don't overwrite 'C2X_DesSpeedOld' with current 'DesSpeed' = 'speed_incident'
                ...
                # Giving back the adjusted attributes to Vissim (note: attribute 'Pos' is readonly)
