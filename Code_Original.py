...
# At simulation second 200, there is a breakdown of a vehicle. At the time of breakdown, the vehicle sends out a warning message.
        ...
        for cnt_C2X_veh in range(len(Veh_C2X_attributes)):
            if Veh_C2X_attributes[cnt_C2X_veh][0] == 'PARKING' and Veh_C2X_attributes[cnt_C2X_veh][1] == 1: # vehicle has an incident (parking routing decision #1)
                ...
                for cnt_Veh_Rec_Message in range(len(Veh_attributes_Rec_Message)):
                    ...
                    if (veh_type_cur == Vehicle_Type_C2X_no_message or veh_type_cur == Vehicle_Type_C2X_HasCurrentMessage) and pos_cur < Pos_Veh_SM and Pos_Veh_SM > pos_C2X_cur: # check if vehicle has C2X & position of C2X message is downstream & there is no other further downstream message active
